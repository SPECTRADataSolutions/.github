...
### Manifest Validation (DEPRECATED `validate-context-manifest.yml` → future `validate-context-manifest` inline)

Previously a reusable workflow; now slated for consolidation into a single inline validator (`validate-context-manifest`) alongside other governance checks.
...
### Reference Pinning (DEPRECATED `pin-refs-and-checksums.yml` → roadmap `stabilise-source-links`)
...
### Anchor Reachability (DEPRECATED `anchor-reachability-and-size.yml` → planned `validate-anchor-integrity`)
...
**Deprecation Notice:** Reusable form deprecated; repositories will invoke a local `validate-context-manifest` workflow (to be added) which embeds schema + policy validation directly.

### Ref Pinning and Drift Detection (see `stabilise-source-links.yml` replacement)
...
### Anchor Reachability and Size (deprecated – superseded by forthcoming `validate-anchor-integrity.yml`)
...
### Validate a Context Manifest (forthcoming)

```bash
# Placeholder – will run local inline workflow once added
gh workflow run validate-context-manifest.yml -f manifestPath="config/contextManifest.yaml"
```

### Manual Ref Pinning Check (use `stabilise-source-links.yml`)

```bash
gh workflow run stabilise-source-links.yml -f dryRun=true -f manifestPath="config/contextManifest.yaml"
```

### Coverage Validation (future `validate-anchor-integrity.yml`)

```bash
# Placeholder until validator lands
gh workflow run validate-anchor-integrity.yml -f coverageThreshold=98 -f manifestPath="config/contextManifest.yaml"
```
