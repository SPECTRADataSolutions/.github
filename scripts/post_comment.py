...
class InitiativeCommentPoster:
    ...
    def post_lessons_comment(
        ...
        # Format the comment
        comment_body = self.format_lessons_comment(lessons_data)
        ...
        try:
            response = requests.post(url, headers=self.headers, json=payload)  # type: ignore
        ...
        ) as e:  # pyright: ignore[reportOptionalMemberAccess]
            print(f"❌ Error posting comment: {e}")
            if hasattr(e, "response") and e.response:
                print(f"Response: {e.response.text}")
            return False

    def format_lessons_comment(self, lessons_data: Dict[str, Any]) -> str:  # type: ignore
        ...
        comment += "\n---\n\n"
        comment += (
            "_This analysis was generated by the SPECTRA lessons automation system. "
        )
        comment += f"Generated at {datetime.now().strftime('%Y-%m-%d %H:%M')} UTC._\n\n"

    def _generate_action_items(self, lessons: Dict[str, Any]) -> List[str]:
        """Generate actionable items based on lessons."""
        action_items: List[str] = []

        # Risk-based actions
        risks: List[str] = lessons.get("risks", [])
        if risks:
            action_items.append(
                "Create risk assessment document covering identified risks"
            )
            action_items.append("Schedule risk review checkpoint with stakeholders")

        # Mitigation-based actions
        mitigations: List[str] = lessons.get("mitigations", [])
        if mitigations:
            action_items.append(
                "Implement proven mitigation strategies from similar initiatives"
            )
            action_items.append("Document mitigation plan for future reference")

        # Success factor actions
        success_factors: List[str] = lessons.get("success_factors", [])
        if success_factors:
            action_items.append(
                "Review success patterns and align initiative plan accordingly"
            )
...
def main():
    ...
    parser.add_argument(
        "--repo-owner", default="SPECTRADataSolutions", help="Repository owner"
    )
    ...
    if args.lessons_file:
        lessons_data = {  # pyright: ignore[reportUnknownVariableType]
            "confidence": 75.5,
            "similar_count": 3,
            "lessons": {
                "risks": [
                    "Timeline constraints due to complexity",
    ...
    if args.issue_number:
        success = poster.post_lessons_comment(
            ...
            args.issue_number,
            lessons_data,
            args.dry_run,  # type: ignore
        )
        return 0 if success else 1
    else:
        # Just show the formatted comment
        comment = poster.format_lessons_comment(lessons_data)  # type: ignore
