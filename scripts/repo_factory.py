#!/usr/bin/env python3
"""
SPECTRA Repository Factory (Deprecated Terminology)

DEPRECATION: Use terminology "Repository Provisioning". This module will remain
as a compatibility layer during the deprecation window. New integrations should
refer to repository_provisioning.py (shim imports the same implementation).
...
class RepositoryFactory:
    ...
    def parse_slash_command(self, comment_body: str) -> Optional[Dict[str, str]]:
    ...
    def validate_command_params(self, params: Dict[str, str]) -> Tuple[bool, List[str]]:
        ...
        if "domain" not in params:
            ...
            errors.append(
                "❌ domain must be single-token camelCase (e.g., 'governance')"
            )
        ...
        if "templateRepo" in params:
            ...
            if not re.match(template_pattern, params["templateRepo"]):
                errors.append(
                    "❌ templateRepo must be in format 'SPECTRADataSolutions/repoName'"
                )
    ...
    def check_repository_exists(self, org: str, repo_name: str) -> bool:
        """Check if repository already exists in the organisation."""
    ...
    def create_repository(
        self, params: Dict[str, str], dry_run: bool = False
        ...
        if dry_run:
            return (
                ...
                ["🔍 DRY RUN - Repository would be created"],
            )
        ...
        try:
            ...
            repo_data = {
                ...
                "has_projects": True,
                "has_wiki": False,
                "auto_init": template_repo
                is None,  # Only auto-init if not using template
            ...
            if template_repo:
                ...
                repo_data.update({"owner": org, "include_all_branches": False})
            else:
    ...
    def _seed_canonical_labels(
        self, org: str, repo_name: str, warnings: List[str]
    ) -> None:
        ...
        try:
            ...
            labels_path = os.path.join(
                os.path.dirname(__file__), "..", ".github", "labels.json"
            )
            if not os.path.exists(labels_path):
                warnings.append(
                    "⚠️ Canonical labels file not found, skipping label seeding"
                )
                return
    ...
    def _add_organizational_metadata(
        self, org: str, repo_name: str, domain: str, warnings: List[str]
    ) -> None:
        """Add .spectra/metadata.yml organisational metadata."""
        ...
        try:
            ...
            metadata_content = "# SPECTRA Organisational Metadata\n"
            ...
            file_data = {
                "message": "Add organisational metadata",
            ...
            if response.status_code not in [200, 201]:
                warnings.append("⚠️ Failed to add organisational metadata")
            else:
                logging.info("✅ Added organisational metadata")

        except Exception as e:
            warnings.append(f"⚠️ Failed to add organisational metadata: {str(e)}")

    def _add_baseline_readme(
        self, org: str, repo_name: str, domain: str, warnings: List[str]
    ) -> None:
        """Add baseline README.md with organisational structure."""
...
Please follow SPECTRA contribution guidelines and organisational standards.
"""
            ...
            if response.status_code == 200:
                ...
                file_data = {
                    "message": "Update README with organisational structure",
                ...
                # Create new README
                file_data = {
                    "message": "Add baseline README with organisational structure",
                    "content": self._encode_base64(readme_content),
                    "branch": "main",
                }
...
def main():
    ...
    if len(sys.argv) < 4:
        print(
            "Usage: python repo_factory.py <comment_body> <repo_owner> <repo_name> <issue_number> [--dry-run]"
        )
        sys.exit(1)
